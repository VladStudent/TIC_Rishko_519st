...
def getMAD(tBlock, aBlock):
    ...
    return np.sum(
        np.abs(np.subtract(tBlock, aBlock))
...
def main(anchorFrame, targetFrame,
         blockSize=16,
         outfile="Results",
         saveOutput=True):
    ...
    for i in range(0, 3):
        ...
        diffFrame = cv2.absdiff(
            anchorFrame_c,
            targetFrame_c
        )

        predictedFrame = blockSearchBody(
            anchorFrame_c,
            targetFrame_c,
            blockSize
        )

        residualFrame = getResidual(
            targetFrame_c,
            predictedFrame
        )
    ...
    if saveOutput:
        ...
        cv2.imwrite(
            f"{outfile}/Difference between frame.png",
            diffFrameRGB
        )

        cv2.imwrite(
            f"{outfile}/Prediction frame.png",
            predictedFrameRGB
        )

        cv2.imwrite(
            f"{outfile}/Residual frame.png",
            residualFrameRGB
        )

        cv2.imwrite(
            f"{outfile}/Restore frame.png",
            restoreFrameRGB
        )
    ...
    plt.bar(
        ...
        width=barWidth,
        edgecolor='grey',
        label='Компенсація руху'
    )

    plt.title(
        f'Ступінь стиснення = '
        f'{round(sum(bitsAnchor)/sum(bitsPredicted), 2)}',
        fontweight='bold',
        fontsize=15
    )
...
if __name__ == "__main__":
    ...
    fr = random.randint(0, 300)

    frame1, frame2 = getFrames(
        'sample4.avi',
        fr,
        fr + 1
    )
